...
// glTF loading demo: Khronos Box.glb on a ground plane. Box.glb is the
// minimal "did the model load?" test asset -- a single textured cube.
// Useful to confirm the cgltf path is intact without the visual
// complexity of MaterialDemo's PBR sphere grid.
class GltfDemo : public LuckyDemos::DemoBase {
  public:
    explicit GltfDemo(SDL_Window *window)
...
LUCKY_DEMO("glTF", "Loads and renders Khronos Box.glb to validate the cgltf path", GltfDemo);
